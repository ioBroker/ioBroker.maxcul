...
function startAdapter(options) {
    options = options || {};
    Object.assign(options, { name: 'maxcul' });
    ...
    adapter.on('stateChange', (id, state) => {
        ...
        if (name === 'display') {
            ...
            max.sendSetDisplayActualTemperature(objects[channel].native.src, state.val);
        }
...
function sendDayProfile(channel) {
    ...
    const daySendArray = Object.keys(timers[channel]);
    const daySend = daySendArray.filter(item=>/^send_/.test(item));
...
function sendInfo(channel) {
    ...
    if (
        (timers[channel].mode !== undefined || timers[channel].desiredTemperature !== undefined) &&
    ...
    if (
        (timers[channel].send_0_saturday !== undefined && timers[channel].send_0_saturday === true) ||
...
function setStates(obj) {
    ...
    for (const state in obj.data) {
        if (!Object.prototype.hasOwnProperty.call(obj.data, state)) {
            continue;
        }
        if (state === 'src') {
            continue;
        ...
        if (
            ...
            adapter.log.debug(
                `${id}: Set initially desiredTemperature after polling: ${
                    timers[`${adapter.namespace}.${id}`].requestRunning
                }`,
            );
            adapter.log.debug(
                `${id}: Set initially mode after polling: ${timers[`${adapter.namespace}.${id}`].requestRunningMode}`,
            );
...
function formatTimeString(timeString) {
    ...
    const timeStringNum = timeString.toString().match(/\d/g);
...
function createThermostat(data, prefix) {
    ...
    for (var n = 0; n <= 6; n++) {
        ...
        for (var i = 1; i <= 13; i++) {
            ...
            if (i <= 9) {
                setPointNumber = `0${setPointNumber}`;
            }
            obj = {
            ...
            obj = {
                _id: `${adapter.namespace}.${data.serial}.weekProfile._${n}_${weekDay}._${
                    setPointNumber
                }_setPointUntilTime`,
                common: {
    ...
    adapter.getForeignState(`${id}.mode`, (err, state) => {
        adapter.getForeignState(`${id}.desiredTemperature`, (err, stateTemp) => {
            if (
                ...
                max.sendDesiredTemperature(src, newVal, newMode, '00', objects[id].native.type);
            }
        });
    });
}
            ...
            adapter.getForeignState(`${id}.vacationConfig.untilDate`, (err, untilDate) => {
                if (state && state.val !== null && state.val !== undefined) {
                    ...
                    if (oldMode === 2) {
                        ...
                        max.sendVacation(src, oldVal, oldMode, untilDate.val, '00', objects[id].native.type);
                    } else {
                        adapter.log.info(`Poll-Timeout: Reset Polling for device ${id} : ${oldMode}, ${oldVal}`);

                        max.sendDesiredTemperature(src, oldVal, oldMode, '00', objects[id].native.type);
                    }
                }
            });
        });
        ...
        thermostatTimer = setInterval(() => {
            ...
            for (const id in objects) {
                if (
                    ...
                    (objects[id].native.type === 1 || objects[id].native.type === 2 || objects[id].native.type === 3)
                ) {
                    if (
                        devices[objects[id].native.src] &&
                        (!devices[objects[id].native.src].lastReceived ||
                            now - devices[objects[id].native.src].lastReceived > adapter.config.scanner * 60000)
                    ) {
                    ...
                    if (devices[objects[id].native.src].lastReceived && timers[id]) {
                        ...
                        adapter.log.debug(
                            `${id}  Request: ${timers[id].requestRunningMode},${
