...
module.exports = function(env) {
    ...
    return CommunicationServiceLayer = (function(superClass) {
        ...
        CommunicationServiceLayer.prototype.connect = function() {
            ...
            this._serialDeviceParser = this._serialDeviceInstance.pipe(new Readline({delimiter: '\n'}));
                   ...
                   _this._serialDeviceParser.on('data', function(data) {
                        ...
                        if (/^\d+\s+\d+$/.test(dataString)) {
                            ...
                            try {
                                ...
                                return env.logger.error('Error in maxcul.js \'creditsReceived\' : ' + error + ' | Raw data from CUL: ' + data);
                            }
                        }
                        ...
                        if (/^V(.*)/.test(dataString)) {
                            ...
                            try {
                                ...
                                return env.logger.error('Error in maxcul.js \'culDataReceived\' : ' + error + ' | Raw data from CUL: ' + data);
                            }
                        } else if (/^LOVF/.test(dataString)) {
                            try {
                                return _this.emit('LOVF', true);
                            } catch(error) {
                                return env.logger.error('Error in maxcul.js \'LOVF\' : ' + error + ' | Raw data from CUL: ' + data);
                            }
                        } else {
        ...
        CommunicationServiceLayer.prototype.writeQueue = async function() {
            ...
            env.logger.debug('Send Packet to CUL: Wait ' + delay + ' after sending ' + command.trim());
                    ...
                    if (err.name === 'TimeoutError') {
                        if (packet.getSendTries() < 3) {
                            ...
                            if (packet.getRawPacket().slice(14, 20) === '123456') {
                                packet.resolve(true);
                                env.logger.debug('Time information has been sent three times, clean message queue state.');
                                return _this.cleanMessageQueueState();
                            } else {
