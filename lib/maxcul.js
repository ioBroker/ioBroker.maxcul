...
module.exports = function (env) {
    ...
    const CommunicationServiceLayer = require('./communication-layer')(env);
    const CulPacket = require('./culpacket')(env);
    ...
    return (MaxDriver = (function (superClass) {
        ...
        MaxDriver.prototype.handleIncommingMessage = function (message) {
            let packet = this.parseIncommingMessage(message);
        ...
        MaxDriver.prototype.generateTimePayload = function () {
            ...
            const payload =
                Sprintf('%02x', prep.year) +
                Sprintf('%02x', prep.day) +
                Sprintf('%02x', prep.hour) +
                Sprintf('%02x', prep.compressedOne) +
                Sprintf('%02x', prep.compressedTwo);
            return payload;
        };
        ...
        MaxDriver.prototype.sendConfig = function (
            dest,
            comfortTemperature,
            ecoTemperature,
            minimumTemperature,
            ...
            const offsetValue = Sprintf('%02x', (parseFloat(offset) + 3.5) * 2);
            ...
            const payload =
                comfortTemperatureValue +
                ecoTemperatureValue +
                maximumTemperaturenValue +
        ...
        MaxDriver.prototype.sendDesiredTemperature = function (dest, temperature, mode, groupId, deviceType) {
            let payloadBinary;
            let payloadHex;
            let temperatureBinary;

            mode = parseInt(mode, 10);
            const modeBin = (function () {
                switch (mode) {
                    case 0: // auto weekly
                        return '00';
                    case 1: // manual
        ...
        MaxDriver.prototype.sendConfigValve = function (
            dest,
            boostDuration,
            boostValvePosition,
            decalcificationDay,
            ...
            if (boostValvePosition > 100) {
                boostValvePosition = 100;
            }
            if (boostValvePosition < 0) {
                boostValvePosition = 0;
            }
            ...
            if (decalcificationDay < 0) {
                decalcificationDay = 0;
            }
            if (decalcificationDay > 6) {
                decalcificationDay = 0;
            }
            decalcificationHour = parseInt(decalcificationHour, 10);
            if (decalcificationHour < 0) {
                decalcificationHour = 0;
            }
            if (decalcificationHour > 23) {
                decalcificationHour = 0;
            }

            maxValveSetting = parseInt(maxValveSetting, 10);
            if (maxValveSetting > 100) {
                maxValveSetting = 100;
            }
            if (maxValveSetting < 0) {
                maxValveSetting = 0;
            }
            ...
            let boost = (boostDuration << 5) | Math.round(boostValvePosition / 5);
            let decalc = (decalcificationDay << 5) | decalcificationHour;
            boost = boost & 0xff;
            decalc = decalc & 0xff;
            ...
            if (maxValveSetting.length < 2) {
                maxValveSetting = `0${maxValveSetting}`;
            }

            valveOffset = valveOffset.toString(16);
            if (valveOffset.length < 2) {
                valveOffset = `0${valveOffset}`;
            }
            ...
            const payloadHex = boost + decalc + maxValveSetting + valveOffset;
        ...
        MaxDriver.prototype.sendProfileDay = function (
            dest,
            weekDay,
            setPTemp1,
            setPTime1,
            ...
            if (setPTemp1 !== 0 && setPTime1 !== '') {
                ...
                if (setPointByteOne.length === 1) {
                    setPointByteOne = `0${setPointByteOne}`;
                }
                setPointByteTwo = setPTime & 0xff;
                setPointByteTwo = setPointByteTwo.toString(16);
                if (setPointByteTwo.length === 1) {
                    setPointByteTwo = `0${setPointByteTwo}`;
                }
                payloadHex = payloadHex + setPointByteOne + setPointByteTwo;
            } else {
                payloadHex = `${payloadHex}4520`;
            }
            if (setPTemp2 !== 0 && setPTime2 !== '') {
                ...
                payloadHex = payloadHex + payloadHex.substr(-4);
            }
        ...
        MaxDriver.prototype.sendProfileDay2 = function (
            dest,
            weekDay,
            setPTemp8,
            setPTime8,
        ...
        MaxDriver.prototype.sendVacation = function (dest, temperature, mode, untilDate, groupId, deviceType) {
            ...
            if (untilDateNum !== null) {
                ...
                const untilYear = untilString.slice(6, 8);
                const untilMonth = untilString.slice(2, 4);
                const untilDay = untilString.slice(0, 2);
                const untilHour = untilString.slice(8, 10);
                const untilMinute = untilString.slice(10, 12);

                temperature = (temperature * 2).toString(2);
                const temperatureBinary = `000000${temperature}`.substr(-6);
                const payloadBinary = modeBin + temperatureBinary;
                payloadHex = Sprintf('%02x', parseInt(payloadBinary, 2));
                let byteOne = (((untilMonth >> 1) << 5) | untilDay).toString(16);
                if (byteOne.length < 2) {
                    byteOne = `0${byteOne}`;
                }
                let byteTwo = (((untilMonth & 0x1) << 7) | untilYear).toString(16);
                if (byteTwo.length < 2) {
                    byteTwo = `0${byteTwo}`;
                }
                let byteThree = (untilHour * 2 + untilMinute / 30).toString(16);
                if (byteThree.length < 2) {
                    byteThree = `0${byteThree}`;
                }
        ...
        MaxDriver.prototype.PairPing = function (packet) {
            ...
            if (this.pairModeEnabled) {
                payloadBuffer = Buffer.from(packet.getRawPayload(), 'hex');
        ...
        MaxDriver.prototype.WallThermostatControl = function (packet) {
            ...
            env.logger.debug(
                `got data from wallthermostat ${packet.getSource()} desired temp: ${desired} - measured temp: ${
                    measured
                }`,
            );
        ...
        MaxDriver.prototype.ThermostatState = function (packet) {
            let calculatedMeasuredTemperature;
            let payloadParser;
            let rawBitData;
            let rawData;
            let rawMode;
            ...
            env.logger.debug(
                `got data from heatingelement ${packet.getSource()} with payload ${packet.getRawPayload()}`,
            );
            const rawPayload = packet.getRawPayload();
            if (rawPayload.length >= 10) {
                rawPayloadBuffer = Buffer.from(rawPayload, 'hex');
                if (rawPayload.length === 10) {
